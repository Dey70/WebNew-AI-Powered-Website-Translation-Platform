...
export function normalizeHostname(hostname) {
  ...
  if (/^https?:\/\//.test(value)) {
    try {
      ...
      // fall through with the raw string if it doesn't actually parse as a URL
    }
  } else {
    value = value.split("/")[0]; // tolerate a bare "example.com/some/path"
  }
...
/**
 * Extracts the effective hostname for a request: the Origin/Referer header is
 * the primary signal (can't be forged by the page's own client-side JS); a
 * caller-supplied hostname (query param or body field) is spoofable
 * corroboration only, used as a fallback when no origin header is present
 * (e.g. same-origin server-rendered requests).
 */
export function effectiveHostnameFromRequest(req, fallbackHostname) {
  ...
  return { originHeader, hostname: originHostname || fallbackHostname };
}

/**
 * Convenience wrapper: resolves auth for a request given a raw api_key and a
 * fallback hostname (from query/body). Returns the same shape as
 * resolveSiteFromApiKey plus the resolved originHeader for CORS echoing.
 */
export async function resolveSiteFromRequest(req, apiKey, fallbackHostname) {
  const { originHeader, hostname } = effectiveHostnameFromRequest(req, fallbackHostname);
  const result = await resolveSiteFromApiKey(apiKey, hostname);
  return { ...result, originHeader, hostname };
}
