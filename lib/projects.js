...
export async function getProject({ userId, id }) {
  ...
  if (error || !data) {
    if (error) console.error("[projects] getProject failed:", error.message);
    return null;
  }
  ...
  return { ...data, owner_email: ownerProfile?.email || null };
}
...
export async function listMembers({ userId, projectId }) {
  ...
  const { data, error } = await supabase
    .from("project_members")
    .select("id, user_id, status, created_at, profiles(email, full_name)")
    .eq("project_id", projectId)
...
export async function inviteMember({ ownerId, projectId, email }) {
  ...
  if (existing) {
    return { ok: false, error: existing.status === "accepted" ? "already_a_member" : "invite_already_pending" };
  }

  const { data, error } = await supabase
    .from("project_members")
    .insert({ project_id: projectId, user_id: profile.id, status: "pending" })
...
export async function listPendingInvitesForUser({ userId }) {
  ...
  if (error) {
    console.error("[projects] listPendingInvitesForUser failed:", error.message);
    return [];
  }
  if (!invites || invites.length === 0) return [];

  // Separate lookups (not an embedded join on a guessed FK constraint name)
  // for the project name and inviting owner's email, same "explicit
  // fetch, then merge" style as listProjects/getProject in this file.
  ...
  return invites.map((invite) => {
    ...
    return {
      ...
      ownerEmail: project ? emailByOwnerId.get(project.owner_id) || null : null,
    };
  });
}

export async function respondToInvite({ userId, projectId, accept }) {
  const supabase = getServiceClient();
  if (!supabase) return { ok: false, error: "not_configured" };

  // Scoped strictly by user_id -- only the invited person can respond to
  // their own invite, an owner (or anyone else) can't accept on their behalf.
  ...
  if (!accept) {
    const { error } = await supabase
      ...
      .eq("user_id", userId);
    if (error) {
      console.error("[projects] respondToInvite (decline) failed:", error.message);
      return { ok: false, error: error.message };
    }
  ...
  if (error) {
    console.error("[projects] respondToInvite (accept) failed:", error.message);
    return { ok: false, error: error.message };
  }
